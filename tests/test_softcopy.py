...
def run_softcopy(source_path: Path, dest_path: Path):
    softcopy.main.main(
        [
            str(source_path),
            str(dest_path),
...
def test_slow_write_and_softcopy(workspace, dummy_zarr_path, create_zarr2_archive):
    input_path = workspace / "input.zarr"
    output_path = workspace / "output.zarr"
    ...
    softcopy_process = ctx.Process(target=run_softcopy, args=(input_path, output_path))
